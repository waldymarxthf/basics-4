...
import { getCookie, setCookie } from "./modules/cookie.mjs";
//-------------------------------------------------------------------------------------
let listOfCities = [];
let uniqueCities = [];
...
async function getData(nameEndPoint, name = "Moscow") {
  const cityName = name;
  ...
  setCookie("lastLocation", `${cityName}`, { "max-age": 3600 });
  return data;
}
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  const lastLocation = getCookie("lastLocation");
  renderStorage();

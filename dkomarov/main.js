...
function render() {
    ...
    function arrayIteration(n) {
        if (n >= todoList.length) return;
        let task = todoList[n];
        task.priority == PRIORITY.HIGH ?
            createItemHigh(taskBlockHigh, task.name, task.status) :
            createItemLow(taskBlockLow, task.name, task.status);
        arrayIteration(n + 1);
    }
    arrayIteration(0);
